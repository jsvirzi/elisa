...
bool ResponseMatrix::initialize() {
	...
	if(h_x_true && h_x_meas) {
		dimensions = 1;
		nbins_true = h_x_true->GetNbinsX() + (overflow ? 2 : 0);
		nbins_meas = h_x_meas->GetNbinsX() + (overflow ? 2 : 0);
	} else if(h_x_y_true && h_x_y_meas) {
		nbins_true = 
			(h_x_y_true->GetNbinsX() + (overflow ? 2 : 0)) * 
			(h_x_y_true->GetNbinsY() + (overflow ? 2 : 0));
		nbins_meas = 
			(h_x_y_meas->GetNbinsX() + (overflow ? 2 : 0)) * 
			(h_x_y_meas->GetNbinsY() + (overflow ? 2 : 0));
		dimensions = 2;
	} else if(h_x_y_z_true && h_x_y_z_meas) {
		nbins_true = 
			(h_x_y_z_true->GetNbinsX() + (overflow ? 2 : 0)) * 
			(h_x_y_z_true->GetNbinsY() + (overflow ? 2 : 0)) * 
			(h_x_y_z_true->GetNbinsZ() + (overflow ? 2 : 0));
		nbins_meas = 
			(h_x_y_z_meas->GetNbinsX() + (overflow ? 2 : 0)) * 
			(h_x_y_z_meas->GetNbinsY() + (overflow ? 2 : 0)) * 
			(h_x_y_z_meas->GetNbinsZ() + (overflow ? 2 : 0));
		dimensions = 3;
	}
